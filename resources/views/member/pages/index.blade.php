...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Pengaturan Pages <a href={{ route('member.pages.create')  }} class="bg-blue-400 p-2 rounded-md text-white text-sm">tambahkan pages</a>
        </h2>
    </x-slot>
                        ...
                        <tbody>
                            @foreach ($data as $key => $value)
                                ...
                                <td class="border px-6 py-4 text-center">
                                    ...
                                    <form class="inline" method="post" onsubmit="return confirm('yakin dihapus?')" action="{{ route('member.pages.destroy',['post'=>$value->id]) }}">
                                        @csrf
                                        @method('delete')
